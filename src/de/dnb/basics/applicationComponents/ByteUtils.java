...
public final class ByteUtils {
	...
	/**
	 * extract the bytes form passed byte array to a new byte array 
	 * beginning with the byte at 
	...
	public static int find(final byte[] original, final byte value) {
		if (original == null)
			throw new IllegalArgumentException(
				"passed byte array must not be null");
	...
	public static int find(
		...
		if (startPos >= original.length - 1)
			throw new IllegalArgumentException(
				"start pos must not exceed length of input byte array");
