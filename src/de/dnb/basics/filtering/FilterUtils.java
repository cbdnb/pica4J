...
public final class FilterUtils {
	...
	/**
	 * Splittet eine Liste in Teillisten. Jedes Element, das predicate erfüllt führt
	 * eine neue Teilliste an. Alle Teillisten sind nicht leer.
	 *
	 ...
	 */
	public static <R, T extends R, S extends T> List<T> newFilteredList(final Iterable<S> source,
	...
	/**
	 * Liefert zwei Listen: eine, die predicate erfüllt, und eine zweite, die
	 * predicate nicht erfüllt.
	 *
	 * @param source    Liste, nicht null.
	 * @param predicate nicht null.
	 * @return Paar zweier Listen.
	 */
	public static <R, T extends R, S extends T> Pair<List<T>, List<T>> divide(final Iterable<S> source,
	...
	/**
	 * Liefert zwei Listen: eine, deren Elemente in membersOfFirstList enthalten
	 * sind und eine, für die das nicht gilt.
	 *
	 * @param source             nicht null
	 ...
	 */
	@SafeVarargs
	public static <T> Pair<List<T>, List<T>> divide(final Iterable<T> source, final T... membersOfFirstList) {
		final Set<T> set = new HashSet<>(Arrays.asList(membersOfFirstList));
		return divide(source, set::contains);
	}

	/**
	 * Entfernt aus einer Collection Elemente, die nicht von filter akzeptiert
	 * werden.
	 *
	 ...
	 */
	public static <E> boolean contains(final Iterable<E> iterable, final Predicate<? super E> predicate) {
		RangeCheckUtils.assertReferenceParamNotNull("collection", iterable);
	...
	/**
	 * Findet den Zeiger auf das erste Element einer iterierbaren Entität, das
	 * gleich element ist. Es kommt equals() zum Einsatz.
	 *
	 * @param iterable nicht null.
	 * @param element  ein zu suchendes Element
	 * @return Zeiger auf das erste gefundenes Element, sonst null.
	 */
	...
	/**
	 * Ersetzt das erste Listenelement, das identisch element ist, durch
	 * replacement. (Es wird == verwendet).
	 *
	 * @param list
	 * @param element
	 * @param replacement
	 ...
	 */
	public static <E, F extends E, G extends E> boolean replaceIdentical(final List<E> list, final F element,
			final G replacement) {
		RangeCheckUtils.assertReferenceParamNotNull("list", list);
	...
	/**
	 * Findet das erste Listenelement, das gleich element ist, und ersetzt es durch
	 * replacement. Es wird equals() verwendet.
	 *
	 * @param list
	 * @param element
	 * @param replacement
	 ...
	 */
	public static <E, F extends E, G extends E> boolean replace(final List<E> list, final F element,
			final G replacement) {
		RangeCheckUtils.assertReferenceParamNotNull("list", list);
		RangeCheckUtils.assertReferenceParamNotNull("element", element);
		RangeCheckUtils.assertReferenceParamNotNull("replacement", replacement);
	...
	/**
	 * Vergleicht 2 Listen, wobei nur die Elemente berücksichtigt werden, die
	 * predicate erfüllen.
	 *
	 * @param iterable1 nicht null
	 * @param iterable2 nicht null
	 * @param predicate nicht null
	 * @return true, wenn nach Entfernen der nicht Erfüllenden die Elemente der
	 *         beiden Iterables Stück für Stück gleich sind.
	 */
	public static <E, F extends E, G extends E> boolean equals(final Iterable<F> iterable1, final Iterable<G> iterable2,
	...
	/**
	 * Ersetzt ein altes Element der Menge durch ein neues an der Position des
	 * alten. Es wird auch ersetzt, wenn gilt element.equals(replacement), da
	 * möglicherweise doch noch Unterschiede bestehen (Variable, die nicht in
	 * equals() eingehen, aber weitere Information enthalten).
	 *
	 ...
	 */
	public static <E, F extends E, G extends E> boolean replace(final LinkedHashSet<E> set, final F element,
			final G replacement) {
	...
	/**
	 * Erzeugt eine neue iterierbare Menge, mit function auf jedes Element von from
	 * angewandt.
	 *
	 ...
	 */
	public static <S, T> ArrayList<T> map(final Iterable<S> source, final Function<? super S, ? extends T> function) {
		RangeCheckUtils.assertReferenceParamNotNull("source", source);
	...
	/**
	 * Erzeugt eine neue iterierbare Menge, mit function auf jedes Element von from
	 * angewandt. Entstehende Null-Werte werden gefiltert.
	 *
	 * @param source   nicht null, kann leer sein.
	 * @param function nicht null. Man muss gegebenenfalls sicherstellen, dass die
	 *                 Funktion auch null-Werte verarbeiten kann.
	 * @param <S>      domain.
	 * @param <T>      Range.
	 *
	 * @return Menge der Werte ohne null. Kann leer sein, nie null
	 */
	public static <S, T> ArrayList<T> mapNullFiltered(final Iterable<S> source,
	...
	/**
	 * Realisiert die Listenoperation höherer Ordnung left fold. Auch bekannt als:
	 * reduce, accumulate, aggregate, compress, oder inject. Berechnet für eine
	 * Liste (a, b , c ...) den Wert f...f(f(f(initial, a), b), c) ...)
	 *
	 * @param iterable iterierbare Menge aus Werten, die mit dem Definitionsbereich
	 *                 des zweiten Arguments von f verträglich sind (Untertypen von
	 *                 X).
	 * @param function Binäre Funktion, deren erstes argument mit dem
	 *                 Wertebereichverträglich ist und deren zweites Argument ein
	 *                 Supertyp des Listentyps ist..
	 ...
	 *
	 */
	public static <X, F> F foldLeft(final Iterable<? extends X> iterable, final BiFunction<F, X, ? extends F> function,
			final F initial) {
		F value = initial;
