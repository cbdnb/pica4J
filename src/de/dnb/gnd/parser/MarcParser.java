...
/**
 * Parst im Augenblick sowohl Normdaten (ziemlich vollständig) und Titeldaten
 * (rudimentär).
 *
 * @author baumann
 *
...
public class MarcParser {
	...
	protected void processDatafield() {
		...
		for (final org.marc4j.marc.Subfield marcSubfield : marcSubfields) {
			...
			if ((actualMarcTag.equals("100") || actualMarcTag.equals("400") || actualMarcTag.equals("700"))
					&& actualMarcSubfieldIndicator == 'a') {
			...
			picaChar = 0;
			/*
			 * $t kommt nur bei Werken vor, dann muss aber der Tag ausgetauscht werden (100
			 * -> 130). Ausserdem ist die bisherige Liste fast nutzlos.
			 */
			if (actualMarcSubfieldIndicator == 't' && searchPicaIndicator(actualMarcSubfieldIndicator) != 't') {
				picaChar = 'a';
			...
			// $9 -> $. (außer bei ISBN)
			else if (actualMarcSubfieldIndicator == '9' && !actualMarcTag.equals("020")) {
				processMarcDollar9();
			}
			...
			try {
				...
				final de.dnb.gnd.parser.Subfield subfield = new de.dnb.gnd.parser.Subfield(indicator, content);
	...
	protected void processMarcX00DollarA() {
		...
		if (actualMarcSubfieldContent.contains(", ")) {
			...
			try {
				final de.dnb.gnd.parser.Subfield subP = new de.dnb.gnd.parser.Subfield(indP, actualMarcSubfieldContent);
	...
	protected void processMarc548DollarA() {
		...
		if (actualMarcSubfieldContent.startsWith(CA)) {
			try {
				...
				de.dnb.gnd.parser.Subfield subD;
				subD = new de.dnb.gnd.parser.Subfield(indD, actualMarcSubfieldContent.substring(CA.length()));
				picaSubfields.add(subD);
			} catch (final IllFormattedLineException e) {
				//
			}
