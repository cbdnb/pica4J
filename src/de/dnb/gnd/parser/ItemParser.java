 ...
 * Die Bibliothek ist durch ihre ILN repräsentiert. Werden Pica3-Daten geparst,
 * die die ELN in der Form
 *  <blockquote>[0101 ] leipzig dnb [101a]</blockquote>
 * an erster Stelle enthalten so wird die ILN aufgrund einer Konkordanz ermittelt.
 * <br>
 * <br>
 * Die Exemplardaten hängen unten am Datensatz. Eingeleitet werden sie mit einem
 * Marker für die Bibliothek. Entweder in Pica3
 *  <blockquote>[0101 ] leipzig dnb [101a]</blockquote> oder in Pica+
 *  <blockquote>101@ ƒa1</blockquote>
 * Die einzelnen Exemplardatensätze der Bibliothek werden eingeleiten von
 * <code>7001</code>  in Pica3 oder von <code>208@/01</code> in Pica+ .
 * <br><br>
 *
 * @author baumann
 *
...
public class ItemParser {
  ...
  private static final TagDB theTagDB = BibTagDB.getDB();
  private static final Pattern newLibPat =
  ...
  */
  public Multimap<String, Record> parseItems(final String raw) {
    final List<String> lines = Arrays.asList(pat.split(raw));
    ...
    zeilen.forEach(zeile ->
    {
      ...
      if (isNewLibrary(zeile)) {
        ...
        if (actualItem != null)
          ...
          actualItem = new Record(idnManifestation, theTagDB);
        setNewLibrary(zeile);
        isNewLib = true;
        return;
      }
      ...
      try {
        final Line line = LineParser.parse(zeile, theTagDB, true);
        if (line == null)
          return;
        if (isNewItem(line)) {
          if (isNewLib) {
      ...
      } catch (final IllFormattedLineException | OperationNotSupportedException
  ...
  private boolean isNewItem(final Line line) {
    return line.getTag().pica3.startsWith("70");
  }
  ...
  private boolean isNewLibrary(final String zeile) {
    ...
    newLibMatcher = newLibPat.matcher(zeile);
  ...
  private void setNewLibrary(final String zeile) {
    if (isPicaPlus) {
      ...
      final String eln = newLibMatcher.group(1).trim();
      final String name = newLibMatcher.group(2).trim();
      final String iln = eln2iln.get(eln);
  ...
  public static void main(final String[] args) {
    ...
    System.out.println(countDNB(record));
  }
  ...
  public static void main1(final String[] args) throws IOException {
    ...
    final RecordReader reader =
      RecordReader.getMatchingReader("U:/pica-0.15.0-x86_64-pc-windows-gnu/samples.dat.gz");
  ...
  /**
   * Hilfsfunktion für DNB-Exemplare. Ob die Item-Datensätze überhaupt Daten enthalten, wird nicht
   * überprüft.
   *
   * @param bib2Items nicht null.
   * @return
  ...
  /**
   * @param idnManifestation  Die idn des bibliographischen Datensatzes,
   *                          der die Manifestation repräsentiert.
   */
  ...
  static String dbString =
    "ELN (Pica3 Anfang)\tILN (Pica+)\tKurzname (lowercase)\tISIL\tSigel (Pica3 Ende)\n"
  ...
  /**
   * ILN zu Externer Bibliothsnummer (ELN). Die ELN wird im Pica3-Format,
   * die ILN im Pica+-Format verwendet.
   *
  ...
  /**
   * Name zu Interner Bibliothsnummer (ILN). Diese wird im Pica+-Format
   * verwendet.
   *
